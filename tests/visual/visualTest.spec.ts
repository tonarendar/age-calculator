...
test.describe('Age Calculator - Visual Regression', () => {
	...
	test('default page UI', { tag: '@visual' }, async ({ page }) => {
	...
	test('success result UI', { tag: '@visual' }, async ({ page }) => {
		const homePage = new HomePage(page);
	...
	test('invalid format error UI', { tag: '@visual' }, async ({ page }) => {
		const homePage = new HomePage(page);
		...
		await expect(page).toHaveScreenshot('age-calculator-invalid-format-error.png', {
			fullPage: true,
			animations: 'disabled',
		});
	});

	test('clear state UI', { tag: '@visual' }, async ({ page }) => {
		const homePage = new HomePage(page);
		await homePage.fillDateOfBirth('1990/05/20');
		await homePage.clickSubmit();
...
test.describe('Age Calculator - Style and Role Checks', () => {
	...
	test('checks expected roles are present', { tag: '@layout' }, async ({ page }) => {
		const heading = page.getByRole('heading', { name: 'Birthdate Age Calculator' });
	...
	test('checks button text color and gradient background', { tag: '@layout' }, async ({ page }) => {
		const submitButton = page.getByRole('button', { name: 'Submit' });

		await expect(submitButton).toHaveCSS('color', 'rgb(255, 255, 255)');
		await expect(submitButton).toHaveCSS('background-image', /linear-gradient/);
	});

	test('checks result text colors for default and error state', { tag: '@layout' }, async ({ page }) => {
		const homePage = new HomePage(page);

		await expect(homePage.getResultText()).toHaveCSS('color', 'rgb(51, 51, 51)');
		...
		await expect(homePage.getResultText()).toHaveCSS('color', 'rgb(231, 76, 60)');
