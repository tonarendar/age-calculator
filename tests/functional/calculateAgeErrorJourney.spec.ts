...
test.describe("Error Journey", { tag: "@error-journey" }, () => {
  ...
  test("1.UI returns error message: Please enter date in YYYY/MM/DD format - for invalid date format input", async ({
    page,
  }) => {
    const homePage = new HomePage(page);
  ...
  test("2.UI returns error message: Please enter date in YYYY/MM/DD format - for Non-Date input", async ({
    page,
  }) => {
    const homePage = new HomePage(page);
  ...
  test("3.UI returns error message: Please enter date in YYYY/MM/DD format - for Empty Date input", async ({
    page,
  }) => {
    const homePage = new HomePage(page);
  ...
  test("4.UI returns error message: Invalid date - for Invalid Date: 0000/00/00", async ({
    page,
  }) => {
    const homePage = new HomePage(page);
    // User Actions
