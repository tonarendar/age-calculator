...
test.describe('UI Security Scenarios', { tag: '@security' }, () => {
  ...
  test('1.UI reject XSS-style input and does not execute scripts on submit', async ({ page }) => {
    const homePage = new HomePage(page);
  ...
  test('3.UI reject oversized malicious-looking payload on submit', async ({ page }) => {
    const homePage = new HomePage(page);
