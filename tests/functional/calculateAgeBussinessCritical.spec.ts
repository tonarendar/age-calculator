...
test.describe("Happy Path scenario", () => {
  ...
  test(
    "1.UI should display birthdate placeholder as YYYY/MM/DD",
    { tag: "@happy-path" },
    async ({ page }) => {
      const homePage = new HomePage(page);
      await expect(homePage.getDateInput()).toHaveAttribute(
        "placeholder",
        "YYYY/MM/DD",
      );
    },
  ...
  test(
    "2.UI should display valid Age for valid Date input",
    { tag: "@happy-path" },
    async ({ page }) => {
      const homePage = new HomePage(page);
      ...
      await expect(homePage.getResultText()).toContainText(
        `You are ${expectedAge} years old.`,
      );
    },
  );

  test(
    "3.UI should display age and custom message for users mm/dd matching current date",
    { tag: "@happy-path" },
    async ({ page }) => {
      const homePage = new HomePage(page);
      ...
      // Assertions
      await expect(homePage.getResultText()).toContainText(
        `You are ${expectedAge} years old. Happy Birthday!`,
      );
    },
  );

  test(
    "4.UI should display custom message for users date one day before current date",
    { tag: "@happy-path" },
    async ({ page }) => {
      const homePage = new HomePage(page);
  ...
  test(
    "5.UI should display custom message (You are not born yet!) for future date",
    { tag: "@happy-path" },
    async ({ page }) => {
      const homePage = new HomePage(page);
  ...
  test(
    "6.UI should display custom message (Are you sure you are born..) for current date",
    { tag: "@happy-path" },
    async ({ page }) => {
      const homePage = new HomePage(page);
      ...
      // Assertions
      await expect(homePage.getResultText()).toContainText(
        "Are you sure you are born today?",
      );
    },
  );

  test(
    "7.UI should display custom message (Wow! you are over 100) for age greater than 100",
    { tag: "@happy-path" },
    async ({ page }) => {
      const homePage = new HomePage(page);
      ...
      // Assertions
      await expect(homePage.getResultText()).toContainText(
        "Wow! You are over 100!",
      );
    },
  );

  test(
    "8.UI should display custom message for age equal to 100",
    { tag: "@happy-path" },
    async ({ page }) => {
      const homePage = new HomePage(page);
      ...
      // Assertions
      await expect(homePage.getResultText()).toContainText(
        "You are 100 years old. Congratulations!",
      );
    },
  );

  test(
    "9.UI should display valid Age - for leap year Date: 2024/02/29",
    { tag: "@happy-path" },
    async ({ page }) => {
      const homePage = new HomePage(page);
  ...
  test(
    "1.UI clears input and result after successful calculation",
    { tag: "@happy-path" },
    async ({ page }) => {
      const homePage = new HomePage(page);
  ...
  test(
    "2.UI clears input and result after format error",
    { tag: "@happy-path" },
    async ({ page }) => {
      const homePage = new HomePage(page);
  ...
  test(
    "3.UI clear is safe on already empty state",
    { tag: "@happy-path" },
    async ({ page }) => {
      const homePage = new HomePage(page);
